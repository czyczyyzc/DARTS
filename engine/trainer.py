...
class Trainer(object):
    def __init__(self, model, netw_optimizer, arch_optimizer, unrolled=False, print_freq=20, distributed=False):
    ...
    def train(self, train_loader, valid_loader, epoch):
        ...
        for i, (data_train, target_train) in enumerate(train_loader):
            ...
            prec1, prec5 = accuracy(logits_, target_train, topk=(1, 5))

            if self.distributed:
                temp = torch.stack([loss_, prec1, prec5])
                dist.reduce(temp, dst=0)
                if dist.get_rank() == 0:
                    temp /= dist.get_world_size()
                loss_, prec1, prec5 = temp

            loss_avg.update(loss_, target_train.size(0))
